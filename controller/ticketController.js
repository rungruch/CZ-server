...
import Products from '../model/Ticket.js'
...
export const get = (req, res) => {
    ...
    Products.findTicket(TicketType,Date)
   .then(product => {
      if(!product) {
        return res.status(404).send({
            error: "Product not found with id " + TicketType
        });            
      }
      res.json(product); // default status = 200
    }).catch(err => {
      return res.status(500).send({
        error: "Error retrieving Product with id " + TicketType
      });
    });
  };;
